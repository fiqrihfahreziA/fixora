...
<tr>
    <td style="text-align:center;">{{ $permintaan->karyawan->nama ?? '-' }}
</td>
    <td style="text-align:center;">{{ $permintaan->karyawan->jabatan ?? '-' }}</td>
...
<table>
    ...
    <tr style="height:90px;">
        ...
        {{-- <td style="text-align:center;">
            @if($permintaan->user->karyawan && $permintaan->user->karyawan->ttd)
                <img src="{{ asset('public/storage/ttd/'.$permintaan->user->karyawan->ttd) }}"
                     style="height:60px;"><br>
            @endif
            ( {{ $permintaan->user->karyawan->nama }} )
        </td> --}}
        {{-- <td style="text-align:center;">
            @if($permintaan->user && $permintaan->user->karyawan)
                @if($permintaan->user->karyawan->ttd)
                    <img src="{{ asset('public/storage/ttd/'.$permintaan->user->karyawan->ttd) }}"
                        style="height:60px;"><br>
                @endif
                ( {{ $permintaan->user->karyawan->nama }} )
        ...
        {{-- <td style="text-align:center;">
            @if($permintaan->karyawan && $permintaan->karyawan->ttd)
                <img src="{{ asset('public/storage/ttd/'.$permintaan->karyawan->ttd) }}"
                    style="height:60px;"><br>
            @endif
            ( {{ $permintaan->karyawan->nama ?? '-' }} )<br>
            <small>
                {{ $permintaan->detailBarang->created_at ? $permintaan->created_at->format('d-m-Y') : '' }}
            </small>
        </td> --}}

        <td style="text-align:center;">
            @if($permintaan->karyawan && $permintaan->karyawan->ttd)
                <img src="{{ asset('storage/ttd/'.$permintaan->karyawan->ttd) }}"
                    style="height:60px;"><br>
            @endif
            ( {{ $permintaan->karyawan->nama ?? '-' }} )<br>

            <small>
                {{ $permintaan->detailBarang->created_at ? $permintaan->detailBarang->created_at->format('d-m-Y H:i:s') : '' }}
            </small>
        </td>
        ...
        {{-- <td style="text-align:center;">
            @if($permintaan->penerima && $permintaan->penerima->ttd)
                <img src="{{ asset('public/storage/ttd/'.$permintaan->penerima->ttd) }}"
                     style="height:60px;"><br>
                ( {{ $permintaan->penerima->nama }} )<br>
                 <small>
                {{ $permintaan->detailBarang->tanggal_verif ? $permintaan->created_at->format('d-m-Y H:i') : '' }}
            </small>
            @else
                <br><br>
                ( Belum Diverifikasi )
        ...
        <td style="text-align:center;">
            @if($permintaan->penerima && $permintaan->penerima->ttd)
                ...
                <small>
                   {{ \Carbon\Carbon::parse($permintaan->detailBarang->tanggal_verif)->format('d-m-Y H:i:s') }}

                </small>

            @else
                <br><br>
                 ( user Belum ada ttd )<br>
                <small>
                     ( {{ $permintaan->penerima->nama ?? 'belum di verif' }} )<br>
        ...
        <td style="text-align:center;">
            @if($permintaan->atasan && $permintaan->atasan->ttd)
                <img src="{{ asset('storage/ttd/'.$permintaan->atasan->ttd) }}"
                     style="height:60px;"><br>
                ( {{ $permintaan->atasan->nama }} )<br>
                 <small>
                {{ \Carbon\Carbon::parse($permintaan->detailBarang->tanggal_acc)->format('d-m-Y H:i:s') }}
            ...
            @else
                ...
                ( user Belum ada ttd )<br>
                <small>
                  ( {{ optional($permintaan->atasan)->nama ?? 'belum ada respon' }} )<br>
                   {{-- {{ \Carbon\Carbon::parse($permintaan->detailBarang->tanggal_verif)->format('d-m-Y H:i:s') }} --}}
