...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="{{ route('atasan.pengadaan') }}" class="btn btn-outline-secondary">
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body">
        ...
        {{-- TAMPILAN LOG STATUS BIDANG --}}
        @if($pengajuan->log_status_bidang)
        <div class="mt-2 p-2 bg-info bg-opacity-10 rounded">
            <small class="text-muted">
                <i class="bi bi-clock-history me-1"></i>
                Log Verifikasi Bidang: 
                <span class="fw-bold text-primary">{{ $pengajuan->log_status_bidang }}</span>
            </small>
        </div>
        @endif
    </div>
    ...
    <div class="col-md-4">
        ...
        <div class="card border-0 shadow-sm bg-light mb-4">
            <div class="card-body">
                <h6 class="fw-bold mb-2">
                    <i class="bi bi-lightbulb text-warning me-2"></i>Petunjuk Verifikasi
                </h6>
        ...
        <div class="card border-0 shadow-sm">
            ...
            <div class="card-body">
                ...
                {{-- FORM VERIFIKASI --}}
                <form action="{{ route('atasan.pengadaan.verifikasi', $pengajuan->id) }}" method="POST" id="verifikasiForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status_verifikasi" class="form-select" id="statusSelect" required>
                            <option value="">-- Pilih Status --</option>
                            <option value="disetujui_kabid">✅ Setujui</option>
                            <option value="ditolak">❌ Ditolak</option>
                            <option value="revisi">📝 Perlu Revisi</option>
                        </select>
                        <small class="text-muted">Status akan disimpan sebagai: 
                            <span id="statusPreview" class="fw-bold text-primary">-</span>
                        </small>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="catatan_verifikasi" id="catatanVerifikasi" 
                                  class="form-control" rows="3" 
                                  placeholder="Tuliskan catatan verifikasi minimal 5 karakter..." 
                                  required></textarea>
                        <small class="text-muted">Minimal 5 karakter</small>
                        <div id="catatanError" class="text-danger small mt-1" style="display: none;">
                            <i class="bi bi-exclamation-circle me-1"></i>
                            Catatan harus diisi minimal 5 karakter
                        </div>
                    </div>
                ...
                <div class="mt-3 p-2 bg-light rounded">
                    <small class="text-muted d-block">
                        <i class="bi bi-clock-history me-1"></i>
                        Status Terakhir: <span class="fw-bold text-primary">{{ $pengajuan->log_status_bidang }}</span>
                    </small>
                </div>
                @endif
            </div>
...
/* Styling form */
#statusSelect:focus, #catatanVerifikasi:focus {
    border-color: #0d6efd;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateStatusPreview() {
        ...
        if (selected && statusMap[selected]) {
            statusPreview.textContent = statusMap[selected];
            statusPreview.className = 'fw-bold text-primary';
        } else {
            statusPreview.textContent = '-';
            statusPreview.className = 'fw-bold text-muted';
        }
    }
    ...
    function validateCatatan() {
        const value = catatanInput.value.trim();
        if (value.length < 5) {
    ...
    catatanInput.addEventListener('input', validateCatatan);
    catatanInput.addEventListener('blur', validateCatatan);
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!statusSelect.value) {
            alert('⚠️ Silakan pilih status verifikasi terlebih dahulu!');
            statusSelect.focus();
            return false;
        }
        ...
        // Validasi catatan
        if (!validateCatatan()) {
            alert('⚠️ Catatan verifikasi harus diisi minimal 5 karakter!');
            catatanInput.focus();
            return false;
        }

        // Konfirmasi
        let confirmMsg = 'Apakah Anda yakin akan ';
        const selectedStatus = statusSelect.value;
        if (selectedStatus === 'disetujui_kabid') {
            confirmMsg += 'MENYETUJUI pengajuan ini?';
        } else if (selectedStatus === 'ditolak') {
            confirmMsg += 'MENOLAK pengajuan ini?';
        } else {
            confirmMsg += 'MEMINTA REVISI pengajuan ini?';
        }
    ...
    // Reset button state jika terjadi error
    if (sessionStorage.getItem('formError')) {
        submitBtn.disabled = false;
        submitBtn.innerHTML = '<i class="bi bi-check-circle me-1"></i> Verifikasi';
